...
public class STBDXT {
    ...
    public static void stb_compress_dxt_block(@NativeType(value="unsigned char *") ByteBuffer byteBuffer, @NativeType(value="unsigned char const *") ByteBuffer byteBuffer2, @NativeType(value="int") boolean bl2, int n2) {
        if (Checks.CHECKS) {
            Checks.check((Buffer)byteBuffer, bl2 ? 16 : 8);
            Checks.check((Buffer)byteBuffer2, 64);
        }
        STBDXT.nstb_compress_dxt_block(MemoryUtil.memAddress(byteBuffer), MemoryUtil.memAddress(byteBuffer2), bl2 ? 1 : 0, n2);
    }
