...
public class NVPointSprite {
    ...
    public static void glPointParameterivNV(@NativeType(value="GLenum") int n2, @NativeType(value="GLint const *") IntBuffer intBuffer) {
        if (Checks.CHECKS) {
            Checks.check((Buffer)intBuffer, 1);
        }
        NVPointSprite.nglPointParameterivNV(n2, MemoryUtil.memAddress(intBuffer));
    }

    public static void glPointParameterivNV(@NativeType(value="GLenum") int n2, @NativeType(value="GLint const *") int[] nArray) {
        long l2 = GL.getICD().glPointParameterivNV;
        if (Checks.CHECKS) {
            Checks.check(l2);
