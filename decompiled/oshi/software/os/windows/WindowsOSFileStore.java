...
extends AbstractOSFileStore {
    ...
    public WindowsOSFileStore(String string, String string2, String string3, String string4, String string5, String string6, String string7, String string8, String string9, long l2, long l3, long l4, long l5, long l6) {
    ...
    public boolean updateAttributes() {
        List<OSFileStore> list = WindowsFileSystem.getLocalVolumes(this.getVolume());
        if (list.isEmpty()) {
            Iterator iterator = this.getMount().length() < 2 ? null : this.getMount().substring(0, 2);
            list = WindowsFileSystem.getWmiVolumes(iterator, false);
        }
        for (OSFileStore oSFileStore : list) {
            if (!this.getVolume().equals(oSFileStore.getVolume()) || !this.getMount().equals(oSFileStore.getMount())) continue;
