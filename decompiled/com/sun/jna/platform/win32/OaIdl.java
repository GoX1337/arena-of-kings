...
public interface OaIdl {
    ...
    extends Structure {
        ...
        public ARRAYDESC(TYPEDESC tYPEDESC, short s2, SAFEARRAYBOUND[] sAFEARRAYBOUNDArray) {
    ...
    extends Structure {
        ...
        public SAFEARRAYBOUND(int n2, int n3) {
            this.cElements = new WinDef.ULONG((long)n2);
            this.lLbound = new WinDef.LONG((long)n3);
            this.write();
        }
    ...
    implements Closeable {
        ...
        public void read() {
            super.read();
            this.rgsabound = this.cDims.intValue() > 0 ? (SAFEARRAYBOUND[])this.rgsabound[0].com_sun_jna_Structure_arr_toArray(this.cDims.intValue()) : new SAFEARRAYBOUND[]{new SAFEARRAYBOUND()};
        }
        ...
        public static ByReference createSafeArray(WTypes.VARTYPE vARTYPE, int ... nArray) {
            SAFEARRAYBOUND[] sAFEARRAYBOUNDArray = (SAFEARRAYBOUND[])new SAFEARRAYBOUND().com_sun_jna_Structure_arr_toArray(nArray.length);
            for (int i2 = 0; i2 < nArray.length; ++i2) {
                sAFEARRAYBOUNDArray[i2].lLbound = new WinDef.LONG(0L);
                sAFEARRAYBOUNDArray[i2].cElements = new WinDef.ULONG((long)nArray[nArray.length - i2 - 1]);
            }
        ...
        public void putElement(Object object, int ... nArray) {
            ...
            for (int i2 = 0; i2 < nArray.length; ++i2) {
                lONGArray[i2] = new WinDef.LONG((long)nArray[nArray.length - i2 - 1]);
            }
        ...
        public int getLBound(int n2) {
            ...
            WinNT.HRESULT hRESULT = OleAuto.INSTANCE.SafeArrayGetLBound(this, new WinDef.UINT((long)n3), lONGByReference);
            COMUtils.checkRC(hRESULT);
        ...
        public int getUBound(int n2) {
            int n3 = this.getDimensionCount() - n2;
            WinDef.LONGByReference lONGByReference = new WinDef.LONGByReference();
            WinNT.HRESULT hRESULT = OleAuto.INSTANCE.SafeArrayGetUBound(this, new WinDef.UINT((long)n3), lONGByReference);
            COMUtils.checkRC(hRESULT);
            return lONGByReference.getValue().intValue();
        }
        ...
        public void redim(int n2, int n3) {
            WinNT.HRESULT hRESULT = OleAuto.INSTANCE.SafeArrayRedim(this, new SAFEARRAYBOUND(n2, n3));
            COMUtils.checkRC(hRESULT);
        }
    ...
    extends Structure {
        ...
        public Date getAsJavaDate() {
            long l2 = (long)this.date * 86400000L + DATE_OFFSET;
            double d2 = 24.0 * Math.abs(this.date - (double)((long)this.date));
            int n2 = (int)d2;
            d2 = 60.0 * (d2 - (double)((int)d2));
            int n3 = (int)d2;
            d2 = 60.0 * (d2 - (double)((int)d2));
            int n4 = (int)d2;
            d2 = 1000.0 * (d2 - (double)((int)d2));
            int n5 = (int)d2;
        ...
        public void setFromJavaDate(Date date) {
            double d2 = date.getTime() - DATE_OFFSET;
            double d3 = d2 / 8.64E7;
            ...
            double d5 = Math.signum(d3) * ((double)(date.getTime() - date2.getTime()) / 8.64E7);
            this.date = d4 + d5;
        }
    ...
    extends IntegerType {
        ...
        public VARIANT_BOOL(long l2) {
            super(2, l2);
        }

        public VARIANT_BOOL(boolean bl2) {
            this(bl2 ? 65535L : 0L);
        }
