...
extends StdCallLibrary {
    ...
    extends Structure {
        ...
        public OaIdl.DISPID[] getRgdispidNamedArgs() {
            OaIdl.DISPID[] dISPIDArray = null;
            int n2 = this.cNamedArgs.intValue();
            if (this.rgdispidNamedArgs != null && n2 > 0) {
                int[] nArray = this.rgdispidNamedArgs.getIntArray(0L, n2);
                dISPIDArray = new OaIdl.DISPID[n2];
                for (int i2 = 0; i2 < n2; ++i2) {
                    dISPIDArray[i2] = new OaIdl.DISPID(nArray[i2]);
                }
            } else {
                dISPIDArray = new OaIdl.DISPID[]{};
            }
            return dISPIDArray;
        }

        public void setRgdispidNamedArgs(OaIdl.DISPID[] dISPIDArray) {
            if (dISPIDArray == null) {
                dISPIDArray = new OaIdl.DISPID[]{};
            }
            this.cNamedArgs = new WinDef.UINT((long)dISPIDArray.length);
            this.rgdispidNamedArgs = new Memory(OaIdl.DISPID.SIZE * dISPIDArray.length);
            int[] nArray = new int[dISPIDArray.length];
            for (int i2 = 0; i2 < nArray.length; ++i2) {
                nArray[i2] = dISPIDArray[i2].intValue();
            }
            this.rgdispidNamedArgs.write(0L, nArray, 0, dISPIDArray.length);
        }
        ...
        public void setArgs(Variant.VARIANT[] vARIANTArray) {
            if (vARIANTArray == null) {
                vARIANTArray = new Variant.VARIANT[]{};
            }
            this.rgvarg = new Variant.VariantArg.ByReference(vARIANTArray);
            this.cArgs = new WinDef.UINT((long)vARIANTArray.length);
        }
