...
public interface NTSecApi {
    ...
    extends Structure {
        ...
        public PLSA_FOREST_TRUST_RECORD[] getEntries() {
            return (PLSA_FOREST_TRUST_RECORD[])this.Entries.com_sun_jna_Structure_arr_toArray(this.RecordCount);
        }
    ...
    extends Structure {
        ...
        public String getString() {
            byte[] byArray = this.Buffer.getByteArray(0L, this.Length);
            if (byArray.length < 2 || byArray[byArray.length - 1] != 0) {
                Memory memory = new Memory(byArray.length + 2);
