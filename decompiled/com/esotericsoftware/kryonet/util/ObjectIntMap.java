...
implements Iterable<Entry<K>> {
    ...
    public ObjectIntMap(int n2, float f2) {
        ...
        this.loadFactor = f2;
        int n3 = ObjectIntMap.tableSize(n2, f2);
    ...
    public void shrink(int n2) {
        ...
        int n3 = ObjectIntMap.tableSize(n2, this.loadFactor);
    ...
    public void ensureCapacity(int n2) {
        int n3 = ObjectIntMap.tableSize(this.size + n2, this.loadFactor);
        if (this.keyTable.length < n3) {
            this.resize(n3);
        }
    }
    ...
    private static int tableSize(int n2, float f2) {
        if (n2 < 0) {
            throw new IllegalArgumentException("capacity must be >= 0: " + n2);
        }
        int n3 = ObjectIntMap.nextPowerOfTwo(Math.max(2, (int)Math.ceil((float)n2 / f2)));
        if (n3 > 0x40000000) {
