...
extends SteamInterface {
    ...
    public boolean fileWrite(String string, ByteBuffer byteBuffer) {
        if (!byteBuffer.isDirect()) {
            throw new SteamException("Direct buffer required!");
        }
        return SteamRemoteStorage.fileWrite(this.pointer, string, byteBuffer, byteBuffer.position(), byteBuffer.remaining());
    }

    public boolean fileRead(String string, ByteBuffer byteBuffer) {
        if (!byteBuffer.isDirect()) {
            throw new SteamException("Direct buffer required!");
        }
        return SteamRemoteStorage.fileRead(this.pointer, string, byteBuffer, byteBuffer.position(), byteBuffer.remaining());
    }
    ...
    public SteamAPICall fileWriteAsync(String string, ByteBuffer byteBuffer) {
        ...
        return new SteamAPICall(SteamRemoteStorage.fileWriteAsync(this.pointer, this.callback, string, byteBuffer, byteBuffer.position(), byteBuffer.remaining()));
    }

    public SteamAPICall fileReadAsync(String string, int n2, int n3) {
        return new SteamAPICall(SteamRemoteStorage.fileReadAsync(this.pointer, this.callback, string, n2, n3));
    }

    public boolean fileReadAsyncComplete(SteamAPICall steamAPICall, ByteBuffer byteBuffer, int n2) {
        return SteamRemoteStorage.fileReadAsyncComplete(this.pointer, steamAPICall.handle, byteBuffer, byteBuffer.position(), n2);
    }
    ...
    public boolean setSyncPlatforms(String string, RemoteStoragePlatform remoteStoragePlatform) {
        return SteamRemoteStorage.setSyncPlatforms(this.pointer, string, remoteStoragePlatform.mask);
    }
    ...
    public boolean fileWriteStreamWriteChunk(SteamUGCFileWriteStreamHandle steamUGCFileWriteStreamHandle, ByteBuffer byteBuffer) {
        return SteamRemoteStorage.fileWriteStreamWriteChunk(this.pointer, steamUGCFileWriteStreamHandle.handle, byteBuffer, byteBuffer.position(), byteBuffer.remaining());
    }
    ...
    public RemoteStoragePlatform[] getSyncPlatforms(String string) {
        int n2 = SteamRemoteStorage.getSyncPlatforms(this.pointer, string);
        return RemoteStoragePlatform.byMask(n2);
    }
    ...
    public int ugcRead(SteamUGCHandle steamUGCHandle, ByteBuffer byteBuffer, int n2, int n3, UGCReadAction uGCReadAction) {
        return SteamRemoteStorage.ugcRead(this.pointer, steamUGCHandle.handle, byteBuffer, byteBuffer.position(), n2, n3, uGCReadAction.ordinal());
    }
    ...
    public static enum RemoteStoragePlatform {
        ...
        private RemoteStoragePlatform(int n3) {
            this.mask = n3;
        }

        static RemoteStoragePlatform[] byMask(int n2) {
            int n3 = Integer.bitCount(n2);
            RemoteStoragePlatform[] remoteStoragePlatformArray = new RemoteStoragePlatform[n3];
            int n4 = 0;
            for (RemoteStoragePlatform remoteStoragePlatform : values) {
                if ((remoteStoragePlatform.mask & n2) == 0) continue;
                remoteStoragePlatformArray[n4++] = remoteStoragePlatform;
            }
            return remoteStoragePlatformArray;
        }
