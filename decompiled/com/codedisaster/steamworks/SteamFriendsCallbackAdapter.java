...
extends SteamCallbackAdapter<SteamFriendsCallback> {
    ...
    void onPersonaStateChange(long l2, int n2) {
        ...
        for (SteamFriends.PersonaChange personaChange : personaChangeValues) {
            if (!SteamFriends.PersonaChange.isSet(personaChange, n2)) continue;
    ...
    void onGameRichPresenceJoinRequested(long l2, String string) {
        ((SteamFriendsCallback)this.callback).onGameRichPresenceJoinRequested(new SteamID(l2), string);
    }

    void onGameServerChangeRequested(String string, String string2) {
        ((SteamFriendsCallback)this.callback).onGameServerChangeRequested(string, string2);
    }
}
