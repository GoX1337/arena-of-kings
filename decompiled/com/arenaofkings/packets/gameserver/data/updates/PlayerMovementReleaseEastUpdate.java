...
public class PlayerMovementReleaseEastUpdate
extends PlayerSnapshot {
    @Override
    public void handle(String string, PlayerSnapshot playerSnapshot, Engine engine) {
        Engine.a("Release east");
        br br2 = ay.ay_a().br_a(string);
        if (br2 != ay.ay_a()) {
            ay.ay_a().br_a(string).com_arenaofkings_packets_gameserver_data_player_shared_SharedAccountData_a().getActive_character_entity().az_a().d(PlayerAction.RUN_EAST);
        } else {
            Engine.a("PacketEASTRELEASE was for me : " + string);
        }
    }
}
